...
// dd_solve_1d, dd_solve_2d): PSNR, the t-schedule, the common IPOPT options, the
// continuation loop itself, and the end-of-run summary. Each driver keeps only
// what is formulation-specific — CLI flags, the partition/owner map, its
...
inline double psnr(const std::vector<double>& clean, const double* u, int m) {
   double mse = 0;
   for (int i = 0; i < m; ++i) { double d = clean[i] - u[i]; mse += d * d; }
   mse /= m;
   return mse == 0 ? 1e9 : 10.0 * std::log10(1.0 / mse);
}

inline std::vector<double> scholtes_schedule(double t0, double tmin, double factor) {
   std::vector<double> sched;
   for (double t = t0; t >= tmin * (1 - 1e-12); t *= factor) sched.push_back(t);
   return sched;
}
...
inline bool init_app(Ipopt::SmartPtr<Ipopt::IpoptApplication>& app,
   ...
   if (solver == "ma57") {
      ...
      app->Options()->SetStringValue(
         "hsllib", hsl ? hsl :
         "/Users/davidvillacis/src/hsl/hsl_ma57-5.3.2/src/.libs/libhsl_ma57.dylib");
   }
...
// (smallest max r·(1−δ)) — NOT the smallest loss: a loose level overfits u_clean
// and is not a real lower-level solution. Stop at the first failed level.
//
...
RunResult run_scholtes(Ipopt::SmartPtr<Ipopt::IpoptApplication> app,
   ...
   for (double t : sched) {
      ...
      const int iters = IsValid(app->Statistics())
                        ? app->Statistics()->IterationCount() : -1;
      ...
      const double value = x.empty() ? 0.0
      ...
      std::cout << "  " << std::setw(9) << t << std::setw(8) << (int)st
                << std::setw(8) << iters << std::setw(14) << comp
                << std::setw(12) << value << std::setw(12) << p.sol_obj_
                << std::setw(9) << cpu << "\n";

      if (ok && comp < res.best_comp) {
         res.best_comp = comp; res.best_x = x; res.best_t = t;
      } else if (!ok) {
...
RunResult run_mu_coupled(Ipopt::SmartPtr<Ipopt::IpoptApplication> app,
                         Ipopt::MpccTNLPBase& p, double t_min, double t_mu_scale,
                         double c_theta, double tol, int print_level,
                         OptimizeFn&& optimize) {
...
inline void print_summary(const Ipopt::MpccTNLPBase& p, const RunResult& r) {
   ...
   std::cout << "  PSNR noisy    : " << psnr(p.uclean_, p.f_.data(), p.n_state)
             << " dB\n";
   std::cout << "  PSNR recon    : " << psnr(p.uclean_, r.best_x.data(), p.n_state)
             << " dB\n";
}

// End-of-run telemetry for the CG interface solve (--interface cg). Printed even
// when the run does not converge — the skip/fallback counts are exactly what
// diagnoses an interface the iteration cannot handle.
inline void print_interface_stats(const std::string& precond, bool alpha_peel,
   ...
   if (st.peeled > 0) std::cout << ", dual-peel " << st.peeled << " duals";
   std::cout << "): "
             << st.solves << " solves, " << st.iters << " total its ("
             << (attempted ? (double)st.iters / attempted : 0.0)
             << " avg over attempted), " << st.nonconv << " did not reach tol="
             << std::scientific << std::setprecision(1) << cg_tol
             << std::defaultfloat << "\n";
   std::cout << "    " << st.skipped_indef
             << " skipped (S indefinite — CG needs SPD), " << st.fallbacks
             << " fell back to the direct MA57 solve, "
             << (attempted - st.fallbacks)
             << " converged iteratively\n";
   if (st.skipped_dead)
      std::cout << "    " << st.skipped_dead << " skipped after a failure on the "
   ...
   std::cout << std::setprecision(3)
             << "    interface wall " << st.wall << " s over "
             << DDArrowheadSolver::omp_threads() << " OpenMP thread(s)\n";
}
...
inline void print_minres_stats(double tol, int lag) {
   ...
   std::cout << "    " << (attempted - st.fallbacks)
             << " accepted iteratively, " << st.fallbacks
             << " fell back to the direct MA57 solve";
   if (st.skipped_dead)
   ...
   if (lag > 1) {
      ...
      for (int a = 0; a < DDArrowheadSolver::InterfaceStats::MAXAGE; ++a) {
         if (!st.age_cnt[a]) continue;
         std::cout << "      age " << std::setw(2) << a << ": "
                   << std::setw(6) << st.age_cnt[a] << " solves, avg "
                   << std::fixed << std::setprecision(1)
                   << (double)st.age_its[a] / st.age_cnt[a] << " its, "
                   << st.age_fb[a] << " fallback(s)\n";
      }
   }
...
inline void print_checksums(Ipopt::MpccTNLPBase& p) {
   ...
   p.eval_jac_g(n, x.data(), true, mcon, (Ipopt::Index)jv.size(), NULL, NULL,
                jv.data());
   p.eval_h(n, x.data(), true, 1.0, mcon, lam.data(), true, (Ipopt::Index)hv.size(),
            NULL, NULL, hv.data());

   double gfmax = 0, gmax = 0, sj = 0, sh = 0;
   for (double v : gf) gfmax = std::max(gfmax, std::abs(v));
   for (double v : g) gmax = std::max(gmax, std::abs(v));
   for (double v : jv) sj += std::abs(v);
   for (double v : hv) sh += std::abs(v);
   ...
   std::cout << "    sum|J|       = " << sj << "   (" << p.jr_.size() << " nnz)\n";
